package com.busanit501.samplejsp_501.connectTest.samplejsp_501.member.service;

import MENU.dto.MenuDTO;
import com.busanit501.samplejsp_501.connectTest.samplejsp_501.member.dto.MemberDTO;
...
public enum MemberService {
    ...
    public List<MemberDTO> getList() {
        ...
        List<MemberDTO> listSample = IntStream.range(0,10).mapToObj(i -> {
            ...
            dto.setMemno((long)i);
            dto.setMemname("Sample Todo Title " + i);
            dto.setMemDate(LocalDate.now());
            return dto;
        }).collect(Collectors.toList());
        return listSample;
    }

    public List<MenuDTO> getList2() {

        List<MenuDTO> samplelist = new ArrayList<MenuDTO>();
        for (int i = 0; i < 10; i++) {
            MenuDTO dto = new MenuDTO();
            dto.setMtno((long)i);
            dto.setMtitle("Sample Todo Title " + i);
            dto.setMdueDate(LocalDate.now());
            samplelist.add(dto);
        }
        return samplelist;
    }
